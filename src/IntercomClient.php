...
class IntercomClient
{
    ...
    public function __construct(string $appIdOrToken, string $password = null, array $extraRequestHeaders = [])
    {
        ...
        $this->export = new IntercomExport($this);
     ...
     * @throws Exception
     */
    public function exportFile($endpoint)
    {
        $uri = $this->uriFactory->createUri($endpoint);
        if (!empty($queryParams)) {
            $uri = $uri->withQuery(http_build_query($queryParams));
        }
        $this->extraRequestHeaders['Accept'] = 'application/octet-stream';
        try {
            $response = $this->sendRequest('GET', $uri);
            $this->setRateLimitDetails($response);
            $stream = $response->getBody()->getContents();
            unset($this->extraRequestHeaders['Accept']);
            return $stream;
        } catch (Exception $e) {
            unset($this->extraRequestHeaders['Accept']);
            throw $e;
